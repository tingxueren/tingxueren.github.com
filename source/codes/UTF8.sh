...
#    echo  $NAUTILUS_SCRIPT_SELECTED_FILE_PATHS > out.txt
#    exit 1  
...
charset_org=$(file -i $file | sed 's/^.*charset=//g')  
...
iconv  -f $charset_org -t $to_charset $file > /tmp/iconv.$$
mv /tmp/iconv.$$ $file
#echo  $NAUTILUS_SCRIPT_SELECTED_FILE_PATHS > out.txt
